...
# Ensure the database and tables exist on import
init_db()

# Shim for rerun(): prefer st.experimental_rerun, else stop execution
try:
...
def authenticate(username: str, password: str):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id, password, is_admin, is_active FROM users WHERE username=?", (username,)
    )
    row = cur.fetchone()
    conn.close()
    if row and row[3]:
...
def get_user(user_id: int):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id, username, email, is_admin, is_active, created_at FROM users WHERE id=?", (user_id,)
    )
    row = cur.fetchone()
    conn.close()
    if row:
...
def list_users(search: str = ""):
    ...
    if search:
        cur.execute(
            "SELECT id, username, email, is_active, is_admin FROM users WHERE username LIKE ? OR email LIKE ?",
            (f"%{search}%", f"%{search}%"),
        )
    else:
...
def log_message(user_id: int, message: str):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO logs (user_id, message, ts) VALUES (?, ?, ?)",
